...
- Repository: New open-source repo (not inside sector7). Sector7 provides the ADR for design governance; implementation lives elsewhere.
- Worker runtime: TypeScript, Wrangler, `@cloudflare/workers-types`.
- Package manager: pnpm.
- Tests: `vitest` for unit tests, Miniflare or `@cloudflare/vitest-pool-workers` for integration.
- dlt pipeline: Python, documented as an integration recipe, not shipped with the Worker.
- Open questions:
